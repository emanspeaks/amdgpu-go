...
- Editor stubs (`cproto/`): C header stubs for `xf86drm.h`, `libdrm/amdgpu.h`, `libdrm/amdgpu_drm.h`, gated by the `cproto` build tag in `amdgpu_linux.go`'s `#cgo` directives. Lets gopls type-check the linux cgo on a Windows host without real libdrm.
...
- `amdgpu-go.code-workspace` configures gopls for linux-primary editor analysis using a zig cross-toolchain:
  ...
  - `CC` set to `zig cc -target x86_64-linux-musl` (no `CXX` because the package has no C++)
  - `ZIG_LIB_DIR=C:/exe/zig-0.17.0/lib`
  - `gopls.build.buildFlags=["-tags=cproto"]` (also `go.buildFlags` and `go.testTags`)
  - This combo lets gopls type-check `amdgpu_linux.go` end-to-end on a Windows host: zig provides Linux glibc/musl headers for `runtime/cgo`, and `cproto/` provides libdrm headers for our package. `amdgpu_windows.go` shows "no packages found" when opened (unavoidable single-context limitation).
